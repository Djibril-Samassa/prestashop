...
class ValidationOrderActions extends DefaultActions
{
    ...
    public function prepareFlowRedirect()
    {
        try {
            ...
            if ('failed' == $sourceRetrieve->status) {
    ...
    */
    public function createOrder()
    {
        ...
        if ('succeeded' != $this->conveyor['status']
            && 'pending' != $this->conveyor['status']
            && 'requires_capture' != $this->conveyor['status']
            && 'requires_action' != $this->conveyor['status']
            && 'processing' != $this->conveyor['status']) {
            return false;
        }

        $message = 'Stripe Transaction ID: ' . $this->conveyor['paymentIntent'];

        if (false !== strpos($this->conveyor['token'], 'pm_')) {
        ...
        try {
            ...
            if ('card' == $intent->payment_method_types[0]
                && 'automatic' != $intent->capture_method
                && null == Configuration::get(Stripe_official::CATCHANDAUTHORIZE)) {
                ProcessLoggerHandler::logInfo(
                    'Capturing card',
                    null,
                    null,
            ...
            if ('requires_capture' == $this->conveyor['status']) {
    ...
    {
        try {
            if ('oxxo' != $this->conveyor['datas']['type']) {
                return true;
            }
    ...
    {
        try {
            if (true === empty($this->conveyor['saveCard']) || true === empty($this->conveyor['cart']->id_customer)) {
                return true;
            }
            ...
            if (true === empty($stripeCustomer->id)) {
    ...
    */
    public function addTentative()
    {
        ...
        try {
            if ('American Express' == $this->conveyor['datas']['type']) {
                $this->conveyor['datas']['type'] = 'amex';
            } elseif ('Diners Club' == $this->conveyor['datas']['type']) {
            ...
            if ('sofort' != Tools::strtolower($cardType)
                && 'sepa_debit' != Tools::strtolower($cardType)
                && 'oxxo' != Tools::strtolower($cardType)) {
    ...
    public function chargeWebhook()
    {
        ...
        switch ($event_type) {
            case Event::PAYMENT_INTENT_SUCCEEDED:
                if ('klarna' == $this->conveyor['event_json']->data->object->charges->data[0]->payment_method_details->type) {
            ...
            case Event::CHARGE_SUCCEEDED:
                if ($order->getCurrentState() != Configuration::get('PS_OS_OUTOFSTOCK_PAID')) {
                    ...
                    if ('oxxo' == $this->conveyor['event_json']->data->object->payment_method_details->type) {
                        $stripePayment = new StripePayment();
